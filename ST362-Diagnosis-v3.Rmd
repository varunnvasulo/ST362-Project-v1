...
The model built from the provided data set predicts NBA points per game from the following set of predictors
...
2.  Creating 2 highly correlated predictors with one having a coefficient of 0
3.  Evaluating a polynomial fit in fitting the model

We first generated dummy variables and combined the categorical predictors into fewer levels. This involved categorizing the positions into 3 broader buckets so as to improve the efficiency of our model. Next, we analysed the predictors present in our data set for correlation, it was noted that eFG. and FG. had a high correlation but eFG. did not contribute significantly in predicting the response variable, as such we opted to not include it in our final model. Finally, we plotted our predictors against the response variable and looked for non-linear relationships within the data. Our findings revealed a quadratic relationship was evident between PTS\~MP and we included that within our final model. The final model we developed that was used to predict the response vector was:
...
To analyse whether or not some of the categories have been combined lets create dummy variables and see what affects this has on the model. We will create a model that doesn't include the categorical predictors represented by dummy variables and perform an ANOVA test to see whether the addition of the categorical variables provides better fit.
...
```{r}
ggplot(data, aes(x = TOV, y = PTS)) +
  geom_point() +
  ggtitle("Scatter Plot: Predictor_x vs. Response")

# Creating scatter plots for all of the predictors against points we can see that there is no logarithmic relationship. Intuitively, it also doesn't make sense in the context of the problem for their to be any logarithmic relationship between any of the predictors and point.
```
...
```{r}
data_2 <- data
data_2$PTS <- data$PTS + 0.0001
model4_v2 <- lm(PTS~.-MP + poly(MP,2), data = data_2)
model_v3 <- boxcox(model4_v2, plotit=FALSE)
optimal_lambda <- model_v3$x[which.max(model_v3$y)]
optimal_lambda
model_v3 <- boxcox(model4_v2)
